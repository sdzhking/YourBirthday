package com.dqztc.yourbirthday.pictimeview;
...
/**
 * 日期时间选择器，可同时选中日期及时间，另见{@link MyDatePicker}
 * <p/>
...
public class DateTimePicker extends WheelPicker {
    ...
    private int startHourSet, startMinuteSet = 0;
    private int endHourSet, endMinuteSet = 59;
    ...
    public void setTimeRangeStart(int startHour, int startMinute) {
        ...
        this.startHourSet = startHour;
        this.startMinuteSet = startMinute;
//        initHourData();
    }
    ...
    public void setTimeRangeEnd(int endHour, int endMinute) {
        ...
        this.endHourSet = endHour;
        this.endMinuteSet = endMinute;
//        initHourData();
    }
    ...
    public void setSelectedItem(int year, int month, int day, int hour, int minute) {
        ...
        reviewHouerDataByYearData(year, month, day);
    }

    private void reviewHouerDataByYearData(int year, int month, int day) {
        if (year == startYear && month == startMonth && day == startDay) {
            startHour = startHourSet;
            startMinute = startMinuteSet;
//            initHourData();
        } else {
            startHour = 0;
            startMinute = 0;
...
//            initHourData();
    ...
    protected View makeCenterView() {
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
            yearView.setLayoutParams(new LinearLayout.LayoutParams(0, MATCH_PARENT, 1.0f));
            yearView.setItems(years, selectedYearIndex, yearLabel);
            yearView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    changeMonthData(selectedYear);
                    monthView.setItems(months, selectedMonthIndex, monthLabel);
                    if (onWheelListener != null) {
                    ...
                    dayView.setItems(days, selectedDayIndex, dayLabel);
                    if (onWheelListener != null) {
                        onWheelListener.onDayWheeled(selectedDayIndex, days.get(selectedDayIndex));
                    }

                    reviewHouerDataByYearData(getIntFromStr(selectedYearStr), getIntFromStr(getSelectedMonth()), getIntFromStr(getSelectedDay()));
...
//            }
        ...
        if (dateMode != NONE) {
            monthView.setLayoutParams(new LinearLayout.LayoutParams(0, MATCH_PARENT, 1.0f));
            monthView.setItems(months, selectedMonthIndex, monthLabel);
            monthView.setOnItemSelectListener(new WheelView.OnItemSelectListener() {
                ...
                public void onSelected(int index) {
                    ...
                    if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                        ...
                        reviewHouerDataByYearData(getIntFromStr(getSelectedYear()), getIntFromStr(getSelectedMonth()), getIntFromStr(getSelectedDay()));
                        hourView.setItems(hours, selectedHour);
                        changeMinuteData(DateUtils.trimZero(selectedHour));
                        minuteView.setItems(minutes, selectedMinute);
                    }
...
//            }
        ...
        if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
            dayView.setLayoutParams(new LinearLayout.LayoutParams(0, MATCH_PARENT, 1.0f));
            dayView.setItems(days, selectedDayIndex, dayLabel);
...
//            }
        ...
        if (timeMode != NONE) {
            hourView.setLayoutParams(new LinearLayout.LayoutParams(0, MATCH_PARENT, 1.0f));
            hourView.setItems(hours, selectedHour);
            hourView.setLabel(hourLabel, false);
...
//            }
            ...
            minuteView.setLayoutParams(new LinearLayout.LayoutParams(0, MATCH_PARENT, 1.0f));
            minuteView.setItems(minutes, selectedMinute);
            minuteView.setLabel(minuteLabel, false);
